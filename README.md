...
- 第一阶段成功后保存检查点，第二阶段失败重试时复用该报告和原 Skill 快照。修改规则后需新建任务。
- `CODEX_REASONING_EFFORT` 默认 `low`，可设置 `medium/high/xhigh`；每阶段仍限时 600 秒。审计记录成功/失败耗时、成功调用 token 用量，以及文件工具耗时和输出字符数，不保存文件工具正文。
